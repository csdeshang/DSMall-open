        ...
        if($config['if_fixed_goods']==1){
            if(!empty($config['goods_id'])){
                ...
                foreach($goods_list as $v1){
                    $groupbuyInfo=$groupbuy_model->getGroupbuyInfoByGoodsCommonID($v1['goods_commonid']);
                    if(!$groupbuyInfo){
                        continue;
                    }
                    $v1['goods_image']=goods_thumb($v1);
                    $v1['order_count']=$groupbuyInfo['groupbuy_buyer_count'];
            ...
            foreach($goods_list as $key => $val){
                $groupbuyInfo=$groupbuy_model->getGroupbuyInfoByGoodsCommonID($val['goods_commonid']);
                    if(!$groupbuyInfo){
                        unset($goods_list[$key]);
                    }
                    $goods_list[$key]['goods_image']=goods_thumb($val);
                    $goods_list[$key]['order_count']=$groupbuyInfo['groupbuy_buyer_count'];
            }
