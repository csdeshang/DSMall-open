        ...
        if($config['if_fixed_goods']==1){
            if(!empty($config['goods_id'])){
                ...
                foreach($goods_list as $v1){
                    $pxianshigoodsInfo=$pxianshigoods_model->getXianshigoodsInfoByGoodsID($v1['goods_id']);
                    if(!$pxianshigoodsInfo){
                        continue;
                    }
                    $v1['goods_image']=goods_thumb($v1);
                    $v1['order_count']=Db::name('ordergoods')->where(array(array('goods_type','=',3),array('goods_id','=',$v1['goods_id'])))->sum('goods_num');
                    $v1['order_ratio']=$v1['goods_storage']?round($v1['order_count']/($v1['order_count']+$v1['goods_storage'])*100,2):100;
            ...
            foreach($goods_list as $key => $val){
                $pxianshigoodsInfo=$pxianshigoods_model->getXianshigoodsInfoByGoodsID($val['goods_id']);
                    if(!$pxianshigoodsInfo){
                        unset($goods_list[$key]);
                    }
                    $goods_list[$key]['goods_image']=goods_thumb($val);
                    $goods_list[$key]['order_count']=Db::name('ordergoods')->where(array(array('goods_type','=',3),array('goods_id','=',$val['goods_id'])))->sum('goods_num');
                    $goods_list[$key]['order_ratio']=$val['goods_storage']?round($goods_list[$key]['order_count']/($goods_list[$key]['order_count']+$val['goods_storage'])*100,2):100;
            }
