...
function goods_video($file) {
    ...
    if (in_array($upload_type['0'], array('alioss', 'cos'))) {//对象存储文件
        ...
        $aliendpoint_type = config('ds_config.aliendpoint_type');
        if ($aliendpoint_type) {
            return HTTP_TYPE . config('ds_config.alioss_endpoint') . '/' . ATTACH_GOODS . '/' . $store_id . '/' . $file;
        } else {
            return 'https://' . config('ds_config.alioss_bucket') . '.' . config('ds_config.alioss_endpoint') . '/' . ATTACH_GOODS . '/' . $store_id . '/' . $file;
        }
    } else {
        $store_id = substr($fname, 0, strpos($fname, '_'));
        if ($file && file_exists(BASE_UPLOAD_PATH . '/' . ATTACH_GOODS . '/' . $store_id . '/' . $file)) {
            return UPLOAD_SITE_URL . '/' . ATTACH_GOODS . '/' . $store_id . '/' . $file;
        } else {
            return '';
        }
    }
...
function goods_resource($file) {
    ...
    if (in_array($upload_type['0'], array('alioss', 'cos'))) {//对象存储文件
        ...
        $aliendpoint_type = config('ds_config.aliendpoint_type');
        if ($aliendpoint_type) {
            return HTTP_TYPE . config('ds_config.alioss_endpoint') . '/' . ATTACH_GOODS_RESOURCE . '/' . $store_id . '/' . $file;
        } else {
            return 'https://' . config('ds_config.alioss_bucket') . '.' . config('ds_config.alioss_endpoint') . '/' . ATTACH_GOODS_RESOURCE . '/' . $store_id . '/' . $file;
        }
    } else {
        $store_id = substr($fname, 0, strpos($fname, '_'));
        if ($file && file_exists(BASE_UPLOAD_PATH . '/' . ATTACH_GOODS_RESOURCE . '/' . $store_id . '/' . $file)) {
            return UPLOAD_SITE_URL . '/' . ATTACH_GOODS_RESOURCE . '/' . $store_id . '/' . $file;
        } else {
            return '';
        }
    }
...
function groupbuy_thumb($image_name = '') {
    ...
    $ext = $temp[1];
    $upload_type = explode('_', $base_name);
    if (in_array($upload_type['0'], array('alioss', 'cos'))) {
        $store_id=$upload_type['1'];
    }else{
        $store_id=$upload_type['0'];
    }
    $url = ds_get_pic(ATTACH_GROUPBUY . DIRECTORY_SEPARATOR . $store_id, $image_name);
    if (!$url) {
        return ds_get_pic(ATTACH_COMMON,config('ds_config.default_goods_image'));
    }
    return $url;
...
function get_order_state($order_info) {
    switch ($order_info['order_state']) {
        ...
        case ORDER_STATE_NEW:
            ...
            break;
        case ORDER_STATE_DEPOSIT:
            $order_state = lang('order_state_deposit');
            break;
        case ORDER_STATE_REST:
            $order_state = lang('order_state_rest');
            break;
        case ORDER_STATE_PAY:
            $order_state = lang('order_state_pay');
            break;
        case ORDER_STATE_PICKUP:
            $order_state = lang('order_state_pickup');
            break;
