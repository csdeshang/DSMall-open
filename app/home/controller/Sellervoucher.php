...
class Sellervoucher extends BaseSeller {
    ...
    public function voucher_list(){
        ...
        $vouchertemplate_id = intval(input('param.vouchertemplate_id'));
        if($vouchertemplate_id<=0){
            $this->error(lang('param_error'), 'Sellervoucher/templatelist');
        }
        
        $condition = array();
        $condition['voucher_store_id'] = session('store_id');
        $condition['vouchertemplate_id'] = $vouchertemplate_id;
        
        $voucher_list = Db::name('voucher')->where($condition)->paginate(['list_rows'=>20,'query' => request()->param()],false);
        
        
        View::assign('voucher_list', $voucher_list->items());
        View::assign('show_page', $voucher_list->render());
        
        $this->setSellerCurMenu('Sellervoucher');
        $this->setSellerCurItem('voucher_list');
        
        return View::fetch($this->template_dir . 'voucher_list');
    }
    ...
    public function templateedit() {
        ...
        $param[] = array('vouchertemplate_store_id', '=', session('store_id'));
        $param[] = array('vouchertemplate_state', '=', 1);
    ...
    private function check_voucher_template_expire($voucher_template_id = '') {
        ...
        $where_array[] = array('vouchertemplate_state', '=', 1);
        Db::name('vouchertemplate')->where($where_array)->update(array('vouchertemplate_state' => 2));
    }
        ...
        switch (request()->action()) {
            ...
            case 'voucher_list':
                $menu_array = array(
                    ...
                    array(
                        'name' => 'voucher_list', 'text' => lang('view_voucher_list'),
