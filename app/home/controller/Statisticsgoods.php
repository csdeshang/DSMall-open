...
class Statisticsgoods extends BaseSeller {
    ...
    public function initialize() {
        ...
        Lang::load(base_path() . 'home/lang/' . config('lang.default_lang') . '/statisticsgoods.lang.php');
    ...
    public function index() {
        ...
        if ($this->choose_gcid > 0) {
            ...
            $where[] = array('gc_parentid_' . $gc_depth, '=', $this->choose_gcid);
        }
        if (trim(input('param.search_gname'))) {
            $where[] = array('goods_name', 'like', "%" . trim(input('param.search_gname')) . "%");
        }
        //查询总条数
        ...
        $stat_ordergoods = $stat_model->statByStatordergoods($where, $field, 5, 0, $this->search_arr['orderby'], 'goods_id');
        View::assign('goodslist', $stat_ordergoods);
        ...
        return View::fetch($this->template_dir . 'goodslist');
    }
    ...
    public function goodsinfo() {
        ...
        if ($goods_id <= 0) {
            View::assign('stat_msg', lang('param_error'));
            echo View::fetch($this->template_dir . $templatesname);
            exit;
        }
        //查询商品信息
        ...
        View::assign('stat_count', $stat_count);
    ...
    public function price() {
        ...
        if ($pricerange_arr) {
            ...
            $ordergooods_list = Db::query('SELECT ' . $field . ' FROM ' . config('database.connections.mysql.prefix') . 'statordergoods WHERE store_id=' . session('store_id') . ' AND order_isvalid=1 AND order_add_time BETWEEN ' . $searchtime_arr[0] . ' AND ' . $searchtime_arr[1] . ($this->choose_gcid > 0 ? (' AND gc_parentid_' . $depth . '=' . $this->choose_gcid) : ''));
            if ($ordergooods_list) {
        ...
        View::assign('statjson', $pricerange_statjson);
        $this->setSellerCurMenu('Statisticsgoods');
        $this->setSellerCurItem('price');
        return View::fetch($this->template_dir . 'goods_price');
    }
    ...
    public function hotgoods() {
        ...
        $this->setSellerCurMenu('Statisticsgoods');
        return View::fetch($this->template_dir . 'hotgoods');
    }
    ...
    protected function getSellerItemList() {
        $menu_array = array(
            array('name' => 'goodslist', 'text' => lang('goods_details'), 'url' => (string) url('Statisticsgoods/index')),
            array('name' => 'price', 'text' => lang('sales_price'), 'url' => (string) url('Statisticsgoods/price')),
            array('name' => 'hotgoods', 'text' => lang('selling_goods'), 'url' => (string) url('Statisticsgoods/hotgoods')),
        );
        return $menu_array;
    }
