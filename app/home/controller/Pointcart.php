...
class Pointcart extends BasePointShop
{
    ...
    public function add() {
        ...
        if (!$data['state']){
            switch ($data['error']){
                case 'ParameterError':
                    echo json_encode(array('done'=>false,'msg'=>$data['msg'],'url'=>(string)url('Pointprod/plist'))); die;
                    break;
                default:
    ...
    public function step1(){
        ...
        if (!$data['state']){
            $this->error($data['msg'],(string)url('Pointprod/index'));
        }
        View::assign('pointprod_arr',$data['data']);

        //实例化收货地址模型（不显示门店地址）
        $address_list = model('address')->getAddressList(array('member_id'=>session('member_id'),'chain_id'=>0), 'address_is_default desc,address_id desc');
    ...
    public function step2() {
        ...
        if (!$data['state']){
            $this->error($data['msg'],(string)url('Pointcart/index'));
        }
        ...
        if (!$data['state']){
            $this->error($data['msg'],(string)url('Pointcart/step1'));
        }
    ...
    public function step3($order_arr=array()) {
        ...
        if ($order_id <= 0){
            $this->error(lang('pointcart_record_error'),(string)url('Index/index'));
        }
        $condition = array();
        $condition[] = array('point_orderid','=',$order_id);
        $condition[] = array('point_buyerid','=',session('member_id'));
        $order_info = model('pointorder')->getPointorderInfo($condition);
        if (!$order_info){
            $this->error(lang('pointcart_record_error'),(string)url('Index/index'));
        }
