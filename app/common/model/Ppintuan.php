...
class Ppintuan extends BaseModel {
    ...
    public function editPintuan($update, $condition) {
        $goods_commonids=Db::name('ppintuan')->where($condition)->column('pintuan_goods_commonid');
        foreach($goods_commonids as $goods_commonid){
            $this->_dGoodsPintuanCache($goods_commonid);
        }
