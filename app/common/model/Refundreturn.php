    ...
    public function getGoodsRefundList($order_list = array(), $order_refund = 0) {
        ...
        if (!empty($order_list) && is_array($order_list)) {
            foreach ($order_list as $key => $value) {
                ...
                if (in_array($order_state, [ORDER_STATE_PAY, ORDER_STATE_PICKUP]) && $payment_code != 'offline') {//已付款未发货的非货到付款订单可以申请取消
                    $order_list[$order_id]['refund'] = '1';
                } elseif ($order_state > ORDER_STATE_PICKUP && !empty($goods_list) && is_array($goods_list)) {//已发货后对商品操作
