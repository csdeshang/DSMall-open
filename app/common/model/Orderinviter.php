    ...
    public function giveMoney($order_id, $type) {
        ...
        if ($orderinviter_list) {
            ...
            foreach ($orderinviter_list as $val) {
                ...
                $inviter = Db::name('inviter')->where(array('inviter_id' => $val['orderinviter_member_id'], 'inviter_state' => 1))->find();
                if ($inviter) {
                    $data = array();
                    ...
                    $goodscommon = Db::name('goodscommon')->where('goods_commonid=' . $val['orderinviter_goods_commonid'])->find();

                    if ($goodscommon) {
                        $goodscommon_data = array();
                        if (!Db::name('orderinviter')->where(array(array('orderinviter_order_id', '=', $order_id), array('orderinviter_goods_commonid', '=', $val['orderinviter_goods_commonid']), array('orderinviter_valid', '<>', 0)))->find()) {
                            //更新商品的分销情况
