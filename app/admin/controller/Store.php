...
class Store extends AdminControl {
    ...
    public function store() {
        ...
        $grade_id = intval(input('get.grade_id'));
        if ($grade_id > 0) {
            $condition[] = array('grade_id', '=', $grade_id);
        }
    ...
    public function store_edit() {
        ...
        if (!request()->isPost()) {
            ...
            if ($result || $store_type) {
                $this->log(lang('ds_edit') . lang('ds_store') . '[' . $store_name . ']', 1);
    ...
    public function reopen_del() {
        ...
        if ($del) {
            ds_del_pic(ATTACH_STORE_JOININ, $reopen_info['storereopen_pay_cert']);
